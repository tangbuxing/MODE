...
def propCounts(data_2, minsize, maxsize):
    ...
    return nums, numsID, current_bw, properties
...
def featureFinder(object, smoothfun, dosmooth, smoothpar, smoothfunargs ,thresh ,\
    ...
    if (any(minsize > 1) or any(maxsize < (xdim[0]*xdim[1]))):
        #统计每个连通域内的格点数
        if (Xfeats != None):
            Xnums = propCounts(np.rot90(Xsm > 310, 1), minsize, maxsize)[0]
            XnumsID = propCounts(np.rot90(Xsm > 310, 1), minsize, maxsize)[1]
            Xfeats = propCounts(np.rot90(Xsm > 310, 1), minsize, maxsize)[2]
            Xprop = propCounts(np.rot90(Xsm > 310, 1), minsize, maxsize)

        if (Yfeats != None):
            Ynums = propCounts(np.rot90(Ysm > 310, 1), minsize, maxsize)[0]
            YnumsID = propCounts(np.rot90(Ysm > 310, 1), minsize, maxsize)[1]
            Yfeats = propCounts(np.rot90(Ysm > 310, 1), minsize, maxsize)[2]
            Yprop = propCounts(np.rot90(Ysm > 310, 1), minsize, maxsize)
